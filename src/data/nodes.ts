...
export interface AppNode {
  ...
  category: string;
  description: string;
  source?: string;       // 来源步骤（如 R02）
  inputs: NodeInput[];
  logic: string;         // 合法 JS，变量名与 inputs[].key 对应
  outputs: NodeOutput[];
  status: 'published' | 'draft';
}

// ─── 初始节点库（从流程文件 V4.1 提取 + 已有节点）────────────────────────────
...
export const INITIAL_NODES: AppNode[] = [
  // ── 供应链与成本 ────────────────────────────────────────────────────────────
  {
    ...
    name: '利润计算器',
    category: '供应链与成本',
    description: '根据成本、售价、销量计算利润和利润率',
    source: '基础',
    inputs: [
  ...
  },
  {
    ...
    name: '采购成本计算',
    category: '供应链与成本',
    description: '双供应商报价取均值，含关税，偏差>15%需第三家（R01）',
    source: 'R01',
    inputs: [
  ...
  },
  {
    ...
    name: '头程物流成本',
    category: '供应链与成本',
    description: '双货代报价取均值，计算单件头程费用（R01）',
    source: 'R01',
    inputs: [
  ...
  // ── ROI 模块 ─────────────────────────────────────────────────────────────────
  {
    ...
    name: '单件净利润',
    category: 'ROI模块',
    description: '售价扣除所有成本和费用后的单件净利润（R02）',
    source: 'R02',
    inputs: [
      { key: '售价', label: '售价', unit: 'USD', defaultValue: 25 },
  ...
  },
  {
    ...
    name: '三套情景P&L',
    category: 'ROI模块',
    description: '保守/中性/乐观三套假设下的净利润和净利率（R02）',
    source: 'R02',
    inputs: [
    ...
    logic:
      // 保守：售价×0.9，ACoS×1.4，退货率×1.6
      '保守净利润 = 售价*0.9 - 单件总成本 - 售价*0.9*ACoS中性*1.4 - 售价*0.9*退货率中性*1.6;\n' +
      '保守净利率 = 保守净利润 / (售价*0.9);\n' +
      // 中性
      '中性净利润 = 售价 - 单件总成本 - 售价*ACoS中性 - 售价*退货率中性;\n' +
      '中性净利率 = 中性净利润 / 售价;\n' +
      // 乐观：售价×1.1，ACoS×0.6，退货率×0.67
  ...
  },
  {
    ...
    name: '首批资金需求',
    category: 'ROI模块',
    description: '采购+头程+广告+认证费用汇总（R03）',
    source: 'R03',
    inputs: [
      { key: '首批件数', label: '首批件数', unit: '件', defaultValue: 500 },
  ...
  },
  {
    ...
    name: '最坏情景损失',
    category: 'ROI模块',
    description: '销量×50%，售价-15%，成本+10% 的压力测试（R04）',
    source: 'R04',
    inputs: [
  ...
  },
  {
    ...
    name: '风险敞口比例',
    category: 'ROI模块',
    description: '总亏损占首批资金的比例，>30%触发否决（R04）',
    source: 'R04',
    inputs: [
  ...
  // ── 市场潜力 ─────────────────────────────────────────────────────────────────
  {
    ...
    name: '月GMV估算',
    category: '市场潜力',
    description: '前100 ASIN总销量 × 平均售价（PL01）',
    source: 'PL01',
    inputs: [
  ...
  },
  {
    ...
    name: '市场趋势判断',
    category: '市场潜力',
    description: '后12月均值 ÷ 前12月均值 = 趋势比率 R（PL01）',
    source: 'PL01',
    inputs: [
  ...
  },
  {
    ...
    name: '季节性强度',
    category: '市场潜力',
    description: '峰谷比>3强季节性，1.5-3中，<1.5弱（PL01）',
    source: 'PL01',
    inputs: [
  ...
  },
  {
    ...
    name: '市场集中度',
    category: '市场潜力',
    description: 'Top10销量占Top100的比例（PL01）',
    source: 'PL01',
    inputs: [
  ...
  // ── 竞争格局 ─────────────────────────────────────────────────────────────────
  {
    ...
    logic:
      '竞争强度得分 = Review壁垒分*0.3 + 年龄壁垒分*0.2 + 品牌壁垒分*0.2 + CPC壁垒分*0.15 + 自营壁垒分*0.15;',
  ...
  // ── 决策信息 ─────────────────────────────────────────────────────────────────
  {
    ...
    name: '决策加权得分',
    category: '决策信息',
    description: '五维度决策矩阵加权评分，≥3.5建议进入（M5）',
    source: 'M5',
    inputs: [
    ...
    logic:
      '决策得分 = 市场潜力分*0.25 + 进入难度分*0.2 + 盈利预期分*0.25 + 需求清晰度分*0.15 + 风险可控分*0.15;',
  ...
  // ── 平台模块 ─────────────────────────────────────────────────────────────────
  {
    ...
    name: '广告费用估算',
    category: '平台模块',
    description: '基于目标ACoS和售价估算广告预算（PL04）',
    source: 'PL04',
    inputs: [
