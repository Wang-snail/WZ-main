/**
 * _shared.ts — 共享的节点类型、Seed 数据和 KV 工具函数
 * 所有 /api/nodes/* Functions 都从这里 import
 * 文件名以 _ 开头，Cloudflare Pages 不会将其当作路由处理
 */
...
export function ok<T>(data: T, status = 200): Response {
  return cors(Response.json({ success: true, data }, { status }));
}

export function err(error: string, status = 400): Response {
  return cors(Response.json({ success: false, error }, { status }));
}
...
export interface Env {
  NODES_KV?: KVNamespace;
}

// ─── KV helpers ───────────────────────────────────────────────────────────────
const KV_KEY = 'nodes';

export async function kvGetNodes(kv: KVNamespace): Promise<AppNode[]> {
  const raw = await kv.get(KV_KEY);
  if (raw) {
    try { return JSON.parse(raw) as AppNode[]; } catch { /* fall through */ }
  }
  // First access: initialise with seed data
  await kv.put(KV_KEY, JSON.stringify(SEED_NODES));
  return SEED_NODES;
}

export async function kvSaveNodes(kv: KVNamespace, nodes: AppNode[]): Promise<void> {
  await kv.put(KV_KEY, JSON.stringify(nodes));
}
